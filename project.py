...
# - ini file consts
PROJECT_INI_FILE_PATH: Final[str] = PROJECT_PATH.joinpath('project.ini').as_posix()
...
class ProjectProperties:
    ...
    def _get_deps_from_pipfile(self, section: str = "default", pipfile_path: str = "Pipfile.lock") -> list[str]:
        with open(self.project_path.joinpath(pipfile_path).as_posix()) as pipfile:
    ...
    def _get_deps_from_requirements(self, requirements_path: str = "requirements.txt") -> list[str]:
        with open(self.project_path.joinpath(requirements_path).as_posix()) as file:
    ...
    def get_sources_and_resources_paths(self) -> list[str]:
        return [
            self.project_path.joinpath(self.sources_path).as_posix(),
            self.project_path.joinpath(self.resources_path).as_posix(),
            self.project_path.joinpath(self.test_sources_path).as_posix(),
            self.project_path.joinpath(self.test_resources_path).as_posix()
        ]
...
class CleanCommand(ProjectCommand):
    ...
    def build_command_line(self, properties: ProjectProperties, args: list[str] | None = None) -> list[str]:
        command_line: list[str] = [sys_executable, 'setup.py', 'clean']
        if args:
...
class RunCommand(ProjectCommand):
    ...
    def build_command_line(self, properties: ProjectProperties, args: list[str] | None = None) -> list[str]:
        command_line: list[str] = [sys_executable, properties.project_path.joinpath('setup.py').as_posix(), 'run']
        if args:
            command_line.extend(args)
        return command_line
...
class TestCommand(ProjectCommand):
    ...
    def build_command_line(self, properties: ProjectProperties, args: list[str] | None = None) -> list[str]:
        command_line: list[str] = [sys_executable, 'setup.py', 'test']
        if args:
            command_line.extend(args)
        return command_line
...
class WheelCommand(ProjectCommand):
    ...
    def build_command_line(self, properties: ProjectProperties, args: list[str] | None = None) -> list[str]:
        command_line: list[str] = [sys_executable, '-m', 'pip', 'wheel', f'--wheel-dir={properties.dist_path}', '.']
        if args:
            command_line.extend(args)
        return command_line
...
class SdistCommand(ProjectCommand):
    ...
    def build_command_line(self, properties: ProjectProperties, args: list[str] | None = None) -> list[str]:
        command_line: list[str] = [sys_executable, 'setup.py', 'sdist']
        if args:
            command_line.extend(args)
        return command_line
...
class UploadCommand(ProjectCommand):
    ...
    def build_command_line(self, properties: ProjectProperties, args: list[str] | None = None) -> list[str]:
        command_line: list[str] = ['twine', 'upload', f'{properties.dist_path}/*']
        if args:
            command_line.extend(args)
        return command_line
...
def run(properties: ProjectProperties) -> None:
    ...
    # Prepare PYTHONPATH
    project_paths: Final[list[str]] = properties.get_sources_and_resources_paths()
