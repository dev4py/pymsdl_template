...
class CleanCommand(ProjectCommand):
    ...
    def build_command_line(self, properties: ProjectProperties, args: list[str] | None = None) -> list[str]:
        clean_script: Final[str] = textwrap.dedent(f"""\
        from shutil import rmtree
        from pathlib import Path
        dist_path : Path = Path('{properties.dist_path}')
        if dist_path.is_dir():
            print(" |- Remove %s directory" % dist_path)
            rmtree(dist_path)
        else:
...
class RunCommand(ProjectCommand):
    ...
    def build_command_line(self, properties: ProjectProperties, args: list[str] | None = None) -> list[str]:
        ...
        run_script: Final[str] = textwrap.dedent(f"""\
