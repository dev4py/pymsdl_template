...
class CleanCommand(ProjectCommand):
    ...
    def build_command_line(self, properties: ProjectProperties, args: list[str] | None = None) -> list[str]:
        command_line: list[str] = [
            sys_executable,
            properties.project_path.joinpath("setup.py").as_posix(),
            "clean"
        ]
        if args:
...
class WheelCommand(ProjectCommand):
    ...
    def build_command_line(self, properties: ProjectProperties, args: list[str] | None = None) -> list[str]:
        command_line: list[str] = [
            ...
            "pip",
            "wheel",
            f"--wheel-dir={properties.project_path.joinpath(properties.dist_path).as_posix()}",
            properties.project_path.as_posix()
        ]
        if args:
            command_line.extend(args)
        return command_line
        ...
        if sys_argv and len(sys_argv) > 1:
            ...
            if not self.__command_dict.get(argv):
                ...
                for argv in sys_argv[2:]:
                    ...
                    if cmd:
                        self._run_process(current_cmd.build_command_line(self.__project_properties, current_args))
                        current_args.clear()
                        current_cmd = cmd
                    else:
